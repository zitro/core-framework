...
Reads a project's engagement-repo repo (resolved via ``project.repo_path``) and emits
each markdown file plus the top-level ``engagement-repo.json`` as ``SourceDoc``s.

EngagementRepo's convention is roughly:
    <repo>/
      engagement-repo.json
      <customer>/
        customer-details.md
...
class VertexSourceAdapter(SourceAdapter):
    ...
    def _read_vertex_json(self, root: Path) -> list[SourceDoc]:
        ...
        return [
            SourceDoc(
                id=f"engagement-repo:engagement-repo.json",
