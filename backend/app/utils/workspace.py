...
3-tier convention so the same code works for engagement-repo repos, core overlays,
and ad-hoc note folders:

  1. ``{root}/{customer-slug}/`` — engagement-repo spec
     (``commercial-software-engineering/engagement-repo`` follows this).
...
def resolve_projects_dir(customer_slug: str, root: Path) -> Path:
    ...
    Order:
      1. ``{root}/{customer_slug}/`` if it exists and is non-empty
         (matches the engagement-repo spec).
...
def default_vertex_target(sources: list[Source]) -> Source | None:
    """Pick a default writable engagement-repo Source if exactly one exists.

    Used by /execute "push to engagement-repo" to preselect a target. Returns
