...
def _resolve_writable_vertex(customer: dict | None) -> tuple[dict | None, str]:
    ...
    writable_vertex = [s for s in sources if s.get("writable") and s.get("role") == "engagement-repo"]
    if not writable_vertex:
        return None, "no writable engagement-repo source on customer"
    if len(writable_vertex) > 1:
        # UI must prompt; we don't pick for the user.
        return None, "multiple writable engagement-repo sources — choose one in UI"
...
async def extract_classify(
    ...
    # EngagementRepo projection preview — best-effort, never raises.
    vertex_paths: list[VertexPathPreview] = []
