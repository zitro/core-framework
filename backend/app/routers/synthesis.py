...
async def synthesize(project_id: str) -> dict:
    ...
    return {
        ...
        "failures": failures,
        "writeback": {"engagement-repo": writeback.to_dict()},
    }
...
async def writeback_vertex(project_id: str) -> dict:
    ...
    engagement-repo repo. No-op (with explanatory payload) when write-back is not
    enabled in ``project.metadata.engagement-repo.write_enabled``."""
    project = await _load_project(project_id)
    artifacts = await _project_artifacts(project_id)
...
async def push_artifact(project_id: str, artifact_id: str) -> dict:
    """Push a single artifact back to the connected engagement-repo repo. Wraps
...
# ── project metadata (engagement-repo toggles, etc.) ────────────────────────────
...
async def update_vertex_settings(project_id: str, payload: VertexSettings) -> dict:
    """Update the project's engagement-repo write-back metadata.

    Stored at ``project.metadata.engagement-repo`` so the existing read paths
    ...
    return {"engagement-repo": (saved.get("metadata") or {}).get("engagement-repo") or {}}
