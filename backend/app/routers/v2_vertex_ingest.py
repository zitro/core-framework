"""EngagementRepo repo ingest endpoints (v2.1).

Adds a "drop anything, we'll file it" capability on top of the read-only
engagement-repo tree viewer. Three endpoints:

- ``POST /{project_id}/engagement-repo/extract`` – text from URL or uploaded file
- ``POST /{project_id}/engagement-repo/classify`` – LLM proposes ``dest_path``
- ``POST /{project_id}/engagement-repo/write``    – writes content to the repo
...
@router.post("/{project_id}/engagement-repo/extract")
...
@router.post("/{project_id}/engagement-repo/classify")
async def vertex_classify(project_id: str, payload: ClassifyRequest) -> dict:
    project = await _load_project(project_id)
    root = _resolve_repo(project)
    if not root:
        raise HTTPException(status_code=404, detail="EngagementRepo repo not available")
...
@router.post("/{project_id}/engagement-repo/write")
async def vertex_write(project_id: str, payload: WriteRequest) -> dict:
    ...
    try:
        ...
        with (root / ".engagement-repo-log.jsonl").open("a", encoding="utf-8") as fh:
            fh.write(log_line + "\n")
    except OSError:
