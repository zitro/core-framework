"""v2.0 endpoints: storyboard image generation + engagement-repo repo tree viewer.
...
# ── engagement-repo repo tree viewer ──────────────────────────────────────────────
...
def _resolve_repo(project: dict) -> Path | None:
    ...
    try:
        ...
        logger.exception("v2.engagement-repo: failed to resolve %s", repo_path)
        return None
...
@router.get("/{project_id}/engagement-repo/tree")
async def vertex_tree(project_id: str) -> dict:
    ...
    if not root:
        return VertexTreeResponse(
            ...
            root=None,
            available=False,
            reason="Project has no resolvable engagement-repo repo_path on disk",
        ).model_dump(mode="json")
...
@router.get("/{project_id}/engagement-repo/file")
async def vertex_file(project_id: str, path: str = Query(..., min_length=1)) -> dict:
    ...
    if target is None or not target.exists() or not target.is_file():
        raise HTTPException(status_code=404, detail="File not found in engagement-repo repo")
