...
def test_broken_citation_detects_unknown_source_id() -> None:
    a = _artifact(
        "a1",
        "problem-statement",
        citations=[Citation(source_id="engagement-repo:gone.md")],
    )
    corpus = Corpus(project_id="p1", docs=[_doc("engagement-repo:still-here.md")])
    signals = detect_broken_citations([a], corpus)
    assert len(signals) == 1
    assert "engagement-repo:gone.md" in signals[0].message
...
def test_broken_citation_silent_when_all_valid() -> None:
    a = _artifact(
        "a1",
        "problem-statement",
        citations=[Citation(source_id="engagement-repo:ok.md")],
    )
    corpus = Corpus(project_id="p1", docs=[_doc("engagement-repo:ok.md")])
...
def test_stale_vs_corpus_flags_newer_source() -> None:
    ...
    a = _artifact(
        "a1",
        "problem-statement",
        citations=[Citation(source_id="engagement-repo:notes.md")],
        updated_at=older,
    )
    corpus = Corpus(
        project_id="p1",
        docs=[_doc("engagement-repo:notes.md", modified=newer)],
    )
...
def test_stale_vs_corpus_silent_when_artifact_newer() -> None:
    ...
    corpus = Corpus(
        project_id="p1",
        docs=[_doc("engagement-repo:notes.md", modified=older)],
    )
