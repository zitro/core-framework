"""v2 endpoint tests: storyboard image generation + engagement-repo tree viewer."""
...
async def test_vertex_tree_unavailable_when_no_repo(client: AsyncClient):
    project = (await client.post("/api/engagements/", json={"name": "X", "customer": "Y"})).json()
    res = await client.get(f"/api/v2/{project['id']}/engagement-repo/tree")
...
async def test_vertex_tree_lists_markdown(client: AsyncClient, tmp_path: Path):
    (tmp_path / "engagement-repo.json").write_text('{"name":"acme"}', encoding="utf-8")
    ...
    res = await client.get(f"/api/v2/{project_id}/engagement-repo/tree")
    assert res.status_code == 200
    body = res.json()
    assert body["available"] is True
    names = {c["name"] for c in body["root"]["children"]}
    assert "engagement-repo.json" in names
...
async def test_vertex_file_returns_content(client: AsyncClient, tmp_path: Path):
    ...
    project_id, _ = await _seed_storyboard(client, repo_root=tmp_path)
    res = await client.get(f"/api/v2/{project_id}/engagement-repo/file", params={"path": "notes.md"})
    assert res.status_code == 200
...
async def test_vertex_file_rejects_traversal(client: AsyncClient, tmp_path: Path):
    ...
    project_id, _ = await _seed_storyboard(client, repo_root=tmp_path)
    res = await client.get(f"/api/v2/{project_id}/engagement-repo/file", params={"path": "../../etc/passwd"})
    assert res.status_code == 404
