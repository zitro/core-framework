 ...
 * v2.0 API client.
 *
 * Wraps the /api/v2 surface (storyboard image generation + engagement-repo repo
...
export const v2Api = {
  ...
  vertexTree: (projectId: string) =>
    request<VertexTreeResponse>(
      `/api/v2/${encodeURIComponent(projectId)}/engagement-repo/tree`,
    ),

  vertexFile: (projectId: string, path: string) =>
    request<VertexFileResponse>(
      `/api/v2/${encodeURIComponent(projectId)}/engagement-repo/file?path=${encodeURIComponent(path)}`,
    ),

  vertexClassify: (projectId: string, body: { content: string; hint?: string; filename?: string }) =>
    request<VertexClassifyResponse>(
      `/api/v2/${encodeURIComponent(projectId)}/engagement-repo/classify`,
  ...
  vertexWrite: (
    ...
    request<VertexWriteResponse>(
      `/api/v2/${encodeURIComponent(projectId)}/engagement-repo/write`,
      { method: "POST", body: JSON.stringify(body) },
    ),
  ...
  vertexExtractUrl: (projectId: string, url: string) => {
    ...
    return v2ApiMultipart<VertexExtractResponse>(
      `/api/v2/${encodeURIComponent(projectId)}/engagement-repo/extract`,
      form,
    );
  },
