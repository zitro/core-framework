...
import { VertexWriteBackToggle } from "@/components/synthesis/engagement-repo-toggle";
...
export default function SynthesisPage() {
  ...
  const loadAll = useCallback(async () => {
    ...
    try {
      ...
      const meta = ((p as { metadata?: { engagement-repo?: { write_enabled?: boolean } } } | null)
  ...
  const onWriteBackVertex = async () => {
    ...
    try {
      ...
      if (!res.enabled) {
        toast.warning(
          "EngagementRepo write-back disabled. Set metadata.engagement-repo.write_enabled=true on the project.",
        );
        return;
      }
      if (res.errors.length > 0) {
        ...
        toast.success(`Pushed ${res.written.length} file(s) to engagement-repo`);
      }
    } catch (err) {
      ...
      <header className="flex items-start justify-between gap-4">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <Button
            ...
            disabled={!hasArtifacts}
            title="Push artifacts as markdown into the connected engagement-repo repo"
          >
