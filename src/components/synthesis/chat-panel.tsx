...
/**
 * ChatPanel — project-wide corpus-grounded chat.
 *
 * Posts to ``/api/synthesis/{pid}/chat`` (single corpus, no specific
 * artifact). Maintains a single client-side session id so consecutive
 * turns join the same thread on the server. The per-artifact grounded
 * chat lives in a different component (see ArtifactChatPanel).
 *
 * Send is the only LLM trigger — there's no on-mount auto-fire.
 */
  ...
  return (
    <Card>
      ...
      <CardContent className="space-y-3">
        <div
          ref={scrollRef}
          className="max-h-[420px] min-h-[120px] overflow-y-auto space-y-3 rounded-md border border-border bg-muted/40 p-3 text-sm"
        >
          {turns.length === 0 ? (
