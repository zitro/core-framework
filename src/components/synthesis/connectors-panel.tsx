...
/**
 * ConnectorsPanel — list of optional source connectors with per-project
 * JSON config. The registry endpoint is a cheap read; the on-mount fetch
 * is safe. Save/clear are user-triggered and round-trip to the backend.
 */
...
const SAMPLE_CONFIG: Record<string, string> = {
  ...
  web: JSON.stringify({ urls: ["https://customer.com/roadmap"] }, null, 2),
...
export function ConnectorsPanel({
  ...
  const [sources, setSources] =
    useState<Record<string, unknown>>(initialSources);
  ...
  async function save(kind: string) {
    ...
    setSaving(kind);
    try {
      const res = await synthesisApi.updateConnectorConfig(
        projectId,
        kind,
        parsed,
      );
      ...
      <Card>
        <CardHeader>
          <CardTitle className="font-heading flex items-center gap-2">
            <Plug className="h-4 w-4" aria-hidden /> Source connectors
          </CardTitle>
        </CardHeader>
        <CardContent>
      ...
      <CardContent className="space-y-4">
        {configurable.length === 0 ? (
          <p className="text-sm text-muted-foreground">
            No optional connectors available.
          </p>
        ) : (
          configurable.map((c) => {
            ...
            return (
              ...
              >
                ...
                <Textarea
                  className="w-full font-mono text-xs min-h-[120px] bg-muted/30"
                ...
                <div className="flex items-center gap-2">
                  ...
                  >
                    <Save className="h-3 w-3 mr-1" aria-hidden /> Save
                  </Button>
                  {configured && (
                    <Button
                      size="sm"
