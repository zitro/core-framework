...
export function StoryboardFrames({ artifact, onUpdate }: Props) {
  ...
  const frames =
    (artifact.body?.frames as StoryboardFrame[] | undefined) ?? [];
  ...
  if (frames.length === 0) {
    return <p className="text-xs italic text-muted-foreground">No frames yet.</p>;
  }
    ...
    <div className="space-y-3">
      <div className="flex items-center justify-between gap-3">
        <div className="text-xs text-muted-foreground">
          ...
          {persona && frames.length > 0 && <span> · </span>}
          <span>
            {frames.length} frame{frames.length === 1 ? "" : "s"}
            {missing > 0
              ? ` · ${missing} missing image${missing === 1 ? "" : "s"}`
              : ""}
          </span>
        </div>
        <Button
        ...
        >
          ...
          {missing > 0
            ? `Generate ${missing} image${missing === 1 ? "" : "s"}`
            : "Images filled"}
        </Button>
      </div>
