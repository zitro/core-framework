...
/**
 * ThreadPanel — comments + free-form notes for a single artifact.
 *
 * Renders the thread as a list (human + AI turns share the same stream),
 * lets the user add or delete a note, and stays a pure view: the parent
 * owns ``comments`` and onChange. The chat *input* lives in
 * <ArtifactChatPanel>; this panel just renders the resulting turns.
 *
 * Posts/deletes are user-triggered; no on-mount LLM call.
 */
...
export function ThreadPanel({
  ...
  const addNote = async () => {
    ...
    setBusy(true);
    try {
      const c = await synthesisApi.threads.postComment(projectId, artifactId, {
        body,
        role: "user",
      });
      onChange([...comments, c]);
  ...
  const confirmDelete = async () => {
    ...
    if (!id) return;
    try {
      await synthesisApi.threads.deleteComment(projectId, artifactId, id);
  ...
  return (
    <div className="flex h-full flex-col gap-3">
      ...
      <ConfirmDialog
        ...
        description="This removes the note from the artifact thread. It can't be undone."
        confirmLabel="Delete"
        destructive
...
function CommentRow({
  ...
  const variant: "default" | "secondary" | "outline" = isAsst
    ? "default"
    : isUser
      ? "secondary"
      : "outline";
