...
/**
 * SourcesPanel — read-only listing of the documents that make up the
 * project corpus. Hides placeholder/blank entries so the panel doesn't
 * advertise junk. Loading is owned by the parent so the panel stays a
 * pure renderer; corpus refresh / connector setup belong elsewhere.
 */

import { Database, ExternalLink } from "lucide-react";
import { Badge } from "@/components/ui/badge";
import { Card, CardContent, CardHeader, CardTitle } from "@/components/ui/card";
import type { SynthesisSources } from "@/types/synthesis";
    ...
    <Card>
      <CardHeader>
        <div className="flex items-center gap-2">
          ...
          <CardTitle className="font-heading text-base">Sources</CardTitle>
        </div>
        {sources && (
      ...
      <CardContent className="text-sm">
        {!sources ? (
          ...
          <p className="text-muted-foreground">
            No labelled source documents. Configure an engagement-repo path
            or a connector below to enrich the corpus.
          </p>
        ) : (
          <ul className="space-y-2">
            ...
            {hiddenCount > 0 && (
              <li className="text-[11px] italic text-muted-foreground">
                {hiddenCount} unlabelled document
