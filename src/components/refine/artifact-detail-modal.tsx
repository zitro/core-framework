  ...
  const pushToVertex = async () => {
    ...
    try {
      ...
      if (res.ok) {
        toast.success(
          res.commit
            ? `Pushed to engagement-repo (${res.commit.slice(0, 7)})`
            : "Pushed to engagement-repo",
        );
      } else {
          ...
          <TabsContent value="chat" className="min-h-0 flex-1 space-y-3">
            <div className="flex items-start justify-between gap-3">
              <p className="text-[11px] text-muted-foreground">
                assistant grounded in this artifact, your engagement context, and the thread below.
              </p>
              <Button
                variant="outline"
                size="sm"
