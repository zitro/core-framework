...
- **Settings -> Connections** tab. Surfaces GitHub engagement-repo setup with a
...
- **Artifact card titles still showing engagement name**: the v2.2.4
  ...
  `Personas for Customer's 24-hour Data-to-Insights Initiative` slipped
...
- **Push-to-engagement-repo toast clarity**: distinct, actionable messages for
...
- **Capture page Company and Web tabs**: re-wired `<CompanyPanel />` and
  `<WebSearchPanel />` into `/capture` after the v2.2 IA refactor left
  them as "Coming soon" placeholders. Both flows (company researcher
  + Bing-backed web search) work again under the new IA.
...
Execute, and one-click push to engagement-repo from any artifact.

### Added

- **Multi-source customers (B+E)**: new `customers` collection holds a
  display name, slug, summary, and a list of `Source` records. Each
  source has a kind (`github | local | folder`), role (`engagement-repo | core |
...
- **Per-artifact push (E3)**: `/api/synthesis/{pid}/artifacts/{aid}/
  ...
  "Push to engagement-repo" button that toasts the commit sha on success.
...
IA collapse, beautiful markdown, and an AI-powered drop zone for the engagement-repo repo. Sidebar shrinks from 14 entries across 5 sections to 8 entries across 3 groups + Settings; every legacy route still resolves through Next.js redirects.

### Added

- **Beautiful markdown rendering**: new `MarkdownView` component (`remark-gfm`, `rehype-slug`, `react-syntax-highlighter`) with prose typography, dark-mode aware code highlighting, bordered/zebra tables, capped reading width. Replaces raw `ReactMarkdown` in the `/engagement-repo` viewer.
- **EngagementRepo drop zone**: side sheet on `/engagement-repo` with paste / file upload / URL modes feeds new endpoints:
  - `POST /api/v2/{project_id}/engagement-repo/extract` — text from URL, `.pdf` (`pypdf`), `.docx` (`python-docx`), `.html` (`trafilatura`).
  - `POST /api/v2/{project_id}/engagement-repo/classify` — strict-JSON LLM call proposes `dest_path` + `filename` against the repo's existing folders; falls back to `inbox/` on low confidence or LLM error.
  - `POST /api/v2/{project_id}/engagement-repo/write` — writes the file with path-traversal guard, 409 on conflict unless `overwrite=true`, audits to `.engagement-repo-log.jsonl` at repo root.
...
- **Sidebar IA**: collapsed from 14 items across 5 sections to 8 items across 3 groups (`WORK`, `INSIGHT`, `REFERENCE`) plus a separate `Settings` entry. EngagementRepo Repo stays conditional on `repo_path`.
...
Methodology parity, image generation, engagement-repo repo viewer, top-level Artifacts page, and a global command palette. Additive release; existing routes and behavior preserved.
...
- **EngagementRepo repo viewer**: `GET /api/v2/{project_id}/engagement-repo/tree` and `/engagement-repo/file?path=` back a new `/engagement-repo` page that renders the connected engagement-repo repo as a tree on the left and rendered markdown on the right. Path traversal is rejected at the API.
...
- **Cmd+K command palette**: global keyboard shortcut (`Cmd/Ctrl+K`) opens a quick-jump palette covering all top-level routes; conditionally surfaces EngagementRepo when the project has a `repo_path`.
- **Sidebar**: top-level Artifacts entry on every project; conditional EngagementRepo Repo entry when a repo is connected.
...
Chat over corpus and a UI toggle for engagement-repo bidirectional write-back.
...
- **EngagementRepo settings endpoint**
  - `PUT /api/synthesis/{project_id}/settings/engagement-repo` — set
    `metadata.engagement-repo.write_enabled` and optional `write_subdir` without
...
- **Frontend**
  ...
  - `components/synthesis/engagement-repo-toggle.tsx` — one-click ON/OFF toggle
    for engagement-repo write-back, persists via the new settings endpoint.
  - Synthesis page mounts the chat panel in the right-hand aside and
    the engagement-repo toggle in the header alongside Push to engagement-repo.
...
Story category, customer-ready Office exports, and bidirectional engagement-repo
...
- **EngagementRepo bidirectional write-back** (`backend/app/synthesis/writers/`) —
  `VertexWriteBack` pushes generated artifacts back into the connected
  engagement-repo repo as markdown under `<repo>/synthesis/<category>/<type>.md`,
  plus an auto-maintained `_index.md`. Opt-in per project via
  `metadata.engagement-repo.write_enabled` (default `false`); never touches files
  outside its own subfolder so customer-authored engagement-repo content is safe.
- **Synthesis API surface**
  ...
  - `POST /api/synthesis/{project_id}/writeback/engagement-repo` — push artifacts.
  - `POST /api/synthesis/{project_id}/synthesize` now also runs engagement-repo
    write-back as part of a full synthesize cycle and returns the result
    in `writeback.engagement-repo`.
- **Frontend** — Synthesis page header gains `.docx`, `.pptx`, and
  **Push to engagement-repo** buttons (alongside Resynthesize). Typed client adds
  ...
  engagement-repo write-back is not enabled on the project.
...
- **Synthesis subsystem (`backend/app/synthesis/`)**
  ...
  - Pluggable source adapters: `engagement-repo` (walks the project's engagement-repo repo for
    `engagement-repo.json` + every markdown file), `local_dir` (additional paths from
